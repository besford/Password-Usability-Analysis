...
data = [un.value, acc.value, password, i1, i2, i3, i4, 0, 0, st, 0]
data = np.reshape(data, (1,11))
log = pd.DataFrame(data, columns = ['username', 'account', 'password', 'i1', 'i2', 'i3', 'i4', 'attempts', 'correct', 'updated', 'FAIL'])
...
#Code edit for max user attempt=3
if(int(df['attempts'].iloc[0])<=3):
    ...
    df['FAIL'] = int(df['FAIL'])+1
    save.loc[(save['username'] == tun.value) & (save['account'] == tacc.value)] = df
    print('Unfortunately, you have exceeded the max attempts for this account, try another!')
